...
__all__ = ['CudaRawCovariance','exponential','gaussian','matern','matern_params']
...
gaussian = {'preamble': "", 'params':{'amp':'{{dtype}}','scale':'{{dtype}}'},
'body': """
...
__const__ __device__ {{dtype}} s[4] = { -25.579105509976461286,212.57260432226544008,-610.69018684944109624,422.69668805777760407 };
...
def matern_params(diff_degree, amp, scale):
    ...
    fl = np.floor(diff_degree)
    rem = diff_degree - fl
    prefac = .5**(diff_degree-1.) / scipy.special.gamma(diff_degree)
    return {'diff_degree': diff_degree,
            'amp' : amp,
            'scale' : scale,
            'snu' : snu,
            'fl' : int(fl),
